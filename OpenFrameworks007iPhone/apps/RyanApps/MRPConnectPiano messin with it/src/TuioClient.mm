...
void TuioClient::ProcessMessage( const ReceivedMessage& msg, const IpEndpointName& remoteEndpoint) {
	try {
		...
		if( strcmp( msg.AddressPattern(), "/tuio/2Dobj" ) == 0 ){
			...
			//std::cout << "Message Contains Cursor Info" << std::endl;
			const char* cmd;
			args >> cmd;
				...
				if (!lateFrame) {
					...
					for (std::list<TuioCursor*>::iterator iter=frameCursors.begin(); iter != frameCursors.end(); iter++) {
						...
						switch (tcur->getTuioState()) {
							case TUIO_REMOVED:
								//std::cout << "TUIO_REMOVED" << std::endl;
								frameCursor = tcur;
...
#else
	//Need to convert const char to LPCWSTR using TEXT()
...
std::list<TuioCursor*> TuioClient::getTuioCursors() {
	...
	//Debug
	/*
	std::cout << "objectlist count " << objectList.size() << std::endl;
	std::cout << "frameObjects count " <<frameObjects.size() << std::endl;
	std::cout << "aliveObjectList count " <<aliveObjectList.size() << std::endl;
	std::cout << "cursorList count " <<cursorList.size() << std::endl;
	std::cout << "frameCursors count " <<frameCursors.size() << std::endl;
	std::cout << "aliveCursorList count " <<aliveCursorList.size() << std::endl;
	*/
