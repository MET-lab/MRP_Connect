...
needsUpdate_(true), currentHighlightedKey_(-1), touchSensingEnabled_(false) {
...
void KeyboardDisplay::render() {
	...
	for(int key = lowestMidiNote_; key <= highestMidiNote_; key++) {
		if(keyShape(key) >= 0) {
			...
			drawBlackKey(0, 0, key == currentHighlightedKey_);
			glTranslatef(-offsetH, -offsetV, 0.0);
		}
	}
...
void KeyboardDisplay::drawWhiteTouch(float x, float y, int shape, float touchLocH, float touchLocV, float touchSize) {
	...
	if(touchLocV < kWhiteKeyFrontBackCutoff && touchLocH >= 0.0) {
		...
		for(int i = 0; i < 360; i += 5) {
			glVertex2f(x + cosf((float)i*3.14159/180.0)*touchSize*kDisplayMaxTouchSize 
					   + touchLocH*kWhiteKeyFrontWidth,
					   y + sinf((float)i*3.14159/180.0)*touchSize*kDisplayMaxTouchSize 
...
void KeyboardDisplay::drawBlackTouch(float x, float y, float touchLocV, float touchSize) {
	glPolygonMode(GL_FRONT_AND_BACK, GL_FILL);
	...
	for(int i = 0; i < 360; i += 5) {
		glVertex2f(x + cosf((float)i*3.14159/180.0)*touchSize*kDisplayMaxTouchSize + kBlackKeyWidth*0.5,
				   y + sinf((float)i*3.14159/180.0)*touchSize*kDisplayMaxTouchSize + kBlackKeyLength*touchLocV);
	}	
